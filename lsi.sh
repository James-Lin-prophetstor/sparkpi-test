...
if [ $# -eq 0 ]
   then
    ...
    echo "encInfo       = Show enclosure information of adapter 0"
    ...
    echo "phdinfo       = Show physical device information"
    echo "ldinfo        = Show logical device information"
    echo ""
   exit
 fi
...
if [ $1 = "phdinfo" ]
    then
        if [ -z $2 ]
            then
                # Show physical deive list
                ${MegaCli} -PDList -a0 |grep -E 'Enclosure Device ID:|Slot Number:|Inquiry Data:|Raw Size:|Link Speed:' |sed "s/Enclosure Device ID:/\n\nEnclosure Device ID:/"
                echo -e "\n\nUSAGE:\n$0 $1 [E:S] ; E = enclosure device ID , S = slot ID ; EX: [32:1] \n\n"
                exit
        fi
        # Show physical device information
        ${MegaCli} -pdInfo -PhysDrv${2} -a0
        exit
fi
    ...
    then
        if [ -z $2 ]
            then
                ...
                ${MegaCli} -LDInfo -Lall -a0  |grep -E 'Virtual Drive|RAID Level|Size'
                echo -e "\n\nUSAGE:\nDetails of one logical device\n$0 $1 <ID> ; ID = Device ID ; EX: 1 \n\n"
                exit
        fi
        # Show one logical device details
        ${MegaCli} -LDInfo -L${2} -a0
        exit
fi
